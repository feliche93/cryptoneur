...
const translateContent = async (
  ...
  const filteredKeysToTranslate = Object.entries(keysToTranslate).reduce<Record<string, any>>(
    (acc, [key, value]) => {
      if (value !== null) {
        acc[key] = value
      }
  ...
  if (languages_code === undefined || languages_code.code !== 'EN') {
  ...
  if (targetId === null) {
    ...
    const filteredExistingTranslation = Object.entries(existingTranslation).reduce<
      Record<string, any>
    >((acc, [key, value]) => {
      if (value !== null) {
        acc[key] = value
    ...
    const translatedKeys = await translateObject(
      filteredExistingTranslation,
      sourceLang,
      targetLang,
    )
  ...
  const updatedTranslation = Object.entries(translatedKeys).reduce<Record<string, any>>(
    (acc, [key, value]) => {
...
export async function POST(request: Request) {
  ...
  const targetLangs = ['de-DE', 'es-ES', 'fr-FR', 'it-IT', 'pt-BR', 'ru-RU', 'zh-CN']
