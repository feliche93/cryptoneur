...
async function runReport(query: object) {
  ...
  let data = response.rows.map((row) => {
    if (row) {
      const name = row.dimensionValues[0]
        ? row.dimensionValues[0]?.value
        : row.metricValues[0]?.oneValue
  ...
  data.length > 1
    ? data.push({
        name: 'changePercent',
        value: ((data[0].value - data[1].value) / data[1].value) * 100,
      })
    : data.push({ change: 0 })
  data.length > 1
    ? data.push({ name: 'changeAbsolut', value: data[0].value - data[1].value })
    : data.push({ changeAbsolut: 0 })
  // console.log(data);
  return data
}

export { activeUsers, activeUsers30Days, averageSessionDuration30Days, runReport }
